...
// @ts-ignore
import { Loading_card } from './loading-card.tsx';
...
const Surgeons = () => {
    ...
    async function getSurgeons() {
        ...
        await axios.get('https://firestore.googleapis.com/v1/projects/fourpeaks-sc/databases/(default)/documents/Surgeons')
            .then((res) => { console.log('inside the function', res.data.documents); window.localStorage.setItem('surgeonData', JSON.stringify(res.data.documents)); setLoading(false) });
    }
